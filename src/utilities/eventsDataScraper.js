	...
	try {
		...
		for (let i = 0; i < 4; ++i) {
			...
			for (const anchor of anchors) {
				...
				const date = await anchor.evaluate((node) =>
				...
				try {
					...
					console.log(
						"\x1b[93m%s\x1b[0m",
						`Modal window did not appear within 15 seconds!`
					);
				}
				...
				if (modalPresence) {
					const eventKeys = await page.$$eval(
						"div.s-game-card__header-sport-name > :nth-child(2)",
						(spans) => spans.map((span) => span.textContent)
					);

					const imgURLs = await page.$$eval(
						"div.s-game-card__header__team-logo img",
						(images) => images.map((img) => img.src)
					);

					const opponents = await page.$$eval(
						"div.s-game-card__header__team-event-info a",
						(anchors) => anchors.map((anchor) => anchor.textContent)
					);

					let locations = await page.$$eval(
						"p.text-theme-muted.s-text-paragraph-small span",
						(spans) => spans.map((span) => span.textContent)
					);
					...
					const stadiums = await page.$$eval(
						"p.text-theme-muted.s-text-paragraph-small",
						(paragraphs) => {
					...
					const times = await page.$$eval(
						"div.s-game-card__header__game-score-time span.s-text-paragraph-small",
						(spans) => spans.map((span) => span.textContent)
					);
					...
					for (let j = 0; j < eventKeys.length; ++j) {
						if (locations[j] === "Evanston, Ill.") {
							...
							}-${eventKeys[j]}-${opponents[j]?.replace(/[0-9]/g, "")}`
								.replace(/\s+/g, "")
								.replace(/\./g, "")
								.replace(/\$/g, "")
								.replace(/\[/g, "")
							...
							const value = {
								date:
									typeof date === "undefined" ? null : new Date(date).getTime(),
								point: Math.floor(Math.random() * 6),
								eventKey:
									typeof eventKeys[j] === "undefined" ? null : eventKeys[j],
								opponent:
									typeof opponents[j] === "undefined" ? null : opponents[j],
								imgURL: typeof imgURLs[j] === "undefined" ? null : imgURLs[j],
								time: typeof times[j] === "undefined" ? null : times[j],
								location:
									typeof stadiums[j][0] === "undefined" ? null : stadiums[j][0],
								locationURL:
									typeof stadiums[j][1] === "undefined" ? null : stadiums[j][1],
							};
