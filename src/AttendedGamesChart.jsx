...
const getFirstDate = (games, window) => {
    window == "Total" ? new Date(games.reduce((a, b) => new Date(a.date) < new Date(b.date) ? a : b).date) : historyLookback[window];
    if (window === "Total") {
        const date = new Date(games.reduce((a, b) => new Date(a.date) < new Date(b.date) ? a : b).date);
