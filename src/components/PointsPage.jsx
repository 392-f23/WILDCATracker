...
const PointsPage = () => {
	...
	let attendedGamesIds = [];
	if (userData.length > 1 && userData[0] && "games-attended" in userData[0]) {
		attendedGamesIds = userData[0]["games-attended"];
	}
	const [data, error] = useDbData("/events/");
	const attendedGames = attendedGamesIds.map((gameId) => data[gameId]);
	...
	const points = attendedGames.reduce(
		(sum, game) => sum + parseInt(game.point),
		0
	);
	...
	return profile.user !== undefined && data !== undefined ? (
		<div className='content'>
			...
			<div className='header'>
				...
				<label
					className={
						collapsed ? "chart-toggle-selected" : "chart-toggle-unselected"
					}
			...
			{collapsed && (
				<div style={{ borderBottom: "2px solid lightgray", padding: "20px" }}>
