...
const validateGameData = (key, val) => {
	switch (key) {
		case "eventKey":
			return /((WSOC|MSOC|WVB|FB|FHOCKEY|WFENC|MSWIM|MBB|WBB){1,1})/.test(val)
				? ""
				: "must be one of the following event keys: WSOC, MSOC, WVB, FB, FHOCKEY, WFENC, MSWIM, MBB, or WBB";
		case "date":
			return /(\d{4}[-]\d{2}[-]\d{2})/.test(val)
				? ""
				: "must be in the form of YYYY-MM-DD, e.g., 2023-10-16";
		case "time":
			return /(\d{1}[:]\d{2}\s(a.m.|p.m.))/.test(val)
				? ""
				: "must follow the structure #:## a.m. (or p.m.), e.g., 7:00 p.m.";
		case "point":
	...
	return (
		<div className='d-flex'>
			...
			<button
				...
				disabled={disabled}
				style={{ background: "rgb(78, 42, 132)", border: "rgb(78, 42, 132)" }}
			>
				Submit
			</button>
